...
def activities(request):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS Weather (city TEXT, status TEXT, humidity INTEGER, temperature INTEGER, wind INTEGER, date TEXT)')
    ...
    if request.method == 'POST':
        ...
        if (x != None):
            if (date == x[5]):
                ...
                if (status == "Rain" or status == "Snow" or temp < 32):
                    ...
                    if (status == "Wind" or status == "Mist" or wind_speed == "It is windy outside." or humidity > 80 and 32 < temp < 50):
                        query_result = google_places.nearby_search(
                        location=city, keyword='',
        ...
        if (status == "Rain" or status == "Snow" or temp < 32):
            ...
            if (status == "Wind" or status == "Mist" or status == "Fog" or wind_speed > 15 or humidity > 80 and 32 < temp < 50):
                query_result = google_places.nearby_search(
                location=city, keyword='',
                types=[types.TYPE_AQUARIUM, types.TYPE_ART_GALLERY, types.TYPE_BOWLING_ALLEY, types.TYPE_MUSEUM,
                       types.TYPE_MOVIE_THEATER, types.TYPE_SHOPPING_MALL])
...
def weather(request):    
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS Weather (city TEXT, status TEXT, humidity INTEGER, temperature INTEGER, wind TEXT, date TEXT)')
...
#        print(row)
    ...
    if request.method == 'POST':
        ...
        cur.execute('INSERT INTO Weather (city, status, humidity, temperature, wind, date) VALUES ( ?, ?, ?, ?, ?, ? )', 
                (city, cityData['status'], cityData['humidity'], cityData['temperature'], cityData['wind_speed'], cityData['date_received']))
        conn.commit()
